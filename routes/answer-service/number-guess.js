  ...
  return new Promise((resolve, reject) => {
    ...
    jsdom.env({
      ...
      features: { QuerySelector:true }, //you need query selector for D3 to work
        ...
        let lines = svg.selectAll('line.q-quiz-answer-chart-line')
          .data(stats)
          .enter()
            .append('line')
              .attr('class', 'q-quiz-answer-chart-line s-color-gray-8')
              .attr('style', d => { return `opacity: ${0.1 * d.count > 0.9 ? 0.9 : 0.1 * d.count}`})
  ...
  return new Promise((resolve, reject) => {
    jsdom.env({
        ...
        features: { QuerySelector:true }, //you need query selector for D3 to work
          ...
          let xDomain = [];
          for (let i = data.min; i <= data.max; i = parseFloat((i + data.step).toFixed(precision))) {
            xDomain.push(i)
          }
          ...
          let yScale = d3.scaleLinear()
              .domain([0, Math.max(200/stats.length, d3.max(stats, d => parseInt(d.count)))])
              .range([height, 0]);
