...
export default class NumberGuessHandler {
  ...
  renderResult(answer) {
    ...
    correctAnswerElement.classList.add(
      "q-quiz-result__number-guess-visual__text"
    );
    correctAnswerElement.classList.add(
      "q-quiz-result__number-guess-visual__text--bottom"
    );
    ...
    if (this.correctAnswer - this.min > (this.max - this.min) / 2) {
      let rightPos = (
        (xScale.range.length - xScale.getIndexOnScale(this.correctAnswer) - 1) *
          stepWidth +
        stepWidth / 2 +
        1
      ).toFixed(1);
      ...
      let leftPos = (
        (xScale.getIndexOnScale(this.correctAnswer) + 1) * stepWidth -
        stepWidth / 2 +
        1
      ).toFixed(1);
