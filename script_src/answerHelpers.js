      ...
      .then(articles => {
        articles
          ...
          .forEach((article, index) => {
            ...
            if (
              articleRecommendations[index].text &&
            ...
            }" target="_blank">${article.metadata.title}</a>${
              punctuation.indexOf(article.metadata.title.slice(-1)) === -1
                ? "."
                : ""
            } </span>
            `;
...
export function renderAdditionalInformationForQuestion(element, correctAnswer) {
  ...
  detailedAnswer.parentNode.insertBefore(
    ...
    detailedAnswer.nextSibling
  );

  let nextQuestionButton = element.querySelector(
    "button.q-quiz-button.q-quiz-button--horizontal.q-quiz-button--right"
  );
