...
export default class MultipleChoiceHandler {
  ...
  renderResult(answer) {
    const checkmark =
      '<svg class="q-quiz-result__answer__checkmark s-color-positive" width="16" height="16" viewBox="0 0 16 16" xmlns="http://www.w3.org/2000/svg"><g fill="none" fill-rule="evenodd"><path fill="none" d="M0 12V0h16v16H0z"/><path d="M0 9l5 5L16 3l-2-2-9 9-3-3-2 2z" fill="currentColor"/></g></svg>';
    const crossmark =
      '<svg class="q-quiz-result__answer__checkmark s-color-negative" width="16" height="16" viewBox="0 0 16 16" viewBox="0 0 16 16" xmlns="http://www.w3.org/2000/svg"><g fill="none" fill-rule="evenodd"><path fill="none" d="M0 12V0h16v16H0z"/><path fill="currentColor" d="M8 10l-5 5-2-2 5-5-5-5 2-2 5 5 5-5 2 2-5 5 5 5-2 2-5-5z"/></g></svg>';

    this.choices = [];
    this.questionElement
      .querySelectorAll(".q-quiz-result__answer > span")
      .forEach(element => {
        this.choices.push(element.textContent);
        let parentNode = element.parentNode;
        let bar = parentNode.querySelector(
  ...
  getMaxAnswersCount(answersStats) {
    let maxAnswersCount = this.choices
      .map(choice => {
        if (
          answersStats.hasOwnProperty("numberOfAnswersPerChoice") &&
          answersStats.numberOfAnswersPerChoice.hasOwnProperty(choice)
        ) {
  ...
  renderResultStats(answer, answersStats) {
    let resultVisualElement = this.questionElement.querySelector(
      ".q-quiz-result-visual"
    );
    let maxAnswersCount = this.getMaxAnswersCount(answersStats);

    this.choices.forEach((choice, index) => {
      let bar = resultVisualElement
        .querySelectorAll(".q-quiz-result__multiple-choice-bar")
        .item(index);
      let percentageWithThisAnswer = 0;
      let barWidthPercentage = 0;
      if (answersStats.numberOfAnswersPerChoice.hasOwnProperty(choice)) {
        percentageWithThisAnswer = (
          (answersStats.numberOfAnswersPerChoice[choice] /
            answersStats.totalAnswers) *
          100
        ).toFixed(0);
        barWidthPercentage = (
          (answersStats.numberOfAnswersPerChoice[choice] / maxAnswersCount) *
          100
        ).toFixed(0);
      }
