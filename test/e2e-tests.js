...
lab.experiment("rendering info route", async () => {
  it("renders a quiz", { plan: 4 }, async () => {
    ...
    expect(response.result.markup).startsWith(
      `<div class="q-quiz s-color-gray-4" id="q-quiz-`
    );
  ...
  it(
    "renders quiz for which answers will not be saved (= is pure)",
    { plan: 3 },
    async () => {
      const fixtureResponse = await server.inject("/fixtures/data");
      const fixtureData = fixtureResponse.result;
...
lab.experiment("answer stats route", () => {
  it(
    "returns stats for a multiple choice question of first fixture quiz",
    { plan: 3 },
    async () => {
      const fixtureResponse = await server.inject("/fixtures/data");
      const fixtureData = fixtureResponse.result;
      ...
      expect(
        statsReponse.result.numberOfAnswersPerChoice["richtig"]
      ).to.be.equal(3);
    }
  );
...
lab.experiment("score route", () => {
  ...
  it(
    "returns score for partially answered questions",
    { plan: 2 },
    async () => {
      const fixtureResponse = await server.inject("/fixtures/data");
      const quiz = fixtureResponse.result[0];
...
lab.experiment("number guess plot route", () => {
  ...
  it(
    "returns 400 for requesting number guess plot with multiple choice question",
    { plan: 2 },
    async () => {
      const fixtureResponse = await server.inject("/fixtures/data");
      const fixtureIndex = 0;
